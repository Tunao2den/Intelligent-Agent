...
{  
    ...
    if(Empty(*frontier)){
	    ...
	    if(child->state.h_n + child->path_cost< (*frontier)->node->state.h_n + child->path_cost){ // Child has lowest cost
	        new_queue->next = *frontier;
            *frontier = new_queue; 
        }
        else{
            for(temp_queue = *frontier; temp_queue->next != NULL; temp_queue = temp_queue->next){
                if(child->state.h_n + child->path_cost < temp_queue->next->node->state.h_n + child->path_cost){ 
                     new_queue->next = temp_queue->next;   
                     temp_queue->next = new_queue;
                     return;
                }                                              
